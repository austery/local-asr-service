...
Status: ✅ Accepted
Context: Mac mini M1 Max (64GB), Python 3.11+, FastAPI
Goal: 构建一个 **Production-Ready** 但保持 **MVP** 规模的本地语音运行时网关，支持多种推理引擎。
---
...
- **集成目标**: Spokenly 和 puresubs 需要一个稳定的 OpenAI-compatible 本地入口，而不是一个新的通用音频模型框架。
...
1.  **严格串行 (Strict Serial Execution)**:
    - **原因**: 针对 M1 Max 优化。ASR 推理极快但显存密集，并发会导致 MPS 图编译抖动和显存争抢 (OOM)。
...
4.  **网关边界 (Gateway Boundary)**:
    - **原因**: 项目的价值是稳定 API、队列、模型生命周期和 Apple Silicon 内存边界。底层 ASR、forced alignment、diarization、VAD、speaker clustering 等模型能力应优先复用 `mlx-audio`、FunASR、WhisperKit 或未来 Swift/CoreML runtime 的成熟实现。
    - **实现**: 本服务只封装 runtime contract 和输出格式，不重新实现模型内部算法。新增能力应先判断是“新模型 runtime contract”还是“本服务 orchestration 责任”。
