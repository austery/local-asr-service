...
> 目标：提升用户体验，支持更复杂的使用场景。

### P3-1: 异步任务API (Async Task API) 🔮 未开始

**场景**: 处理超长音频（2-8小时）时，同步API会导致长时间等待，用户体验不佳。

**问题**:
- 当前同步API适用于 < 2小时音频，但对于3小时+的音频（如长播客、会议录音）：
  - 处理时间可能达到 5-10 分钟
  - HTTP 连接长时间占用
  - 用户无法获知处理进度
  - 网络不稳定可能导致连接中断
...
**设计**:
- **提交任务**: `POST /v1/audio/transcriptions?async=true` → 返回 `task_id`
- **查询状态**: `GET /v1/tasks/{task_id}` → 返回 `{status, progress, result}`
- **任务状态**: `queued` → `processing` → `completed` / `failed`
- **存储方案**: 内存存储（简单场景）或 Redis（多实例场景）
- **结果缓存**: 完成的结果保留 1-24 小时可查询
...
**优先级**: 🟡 中等 — 当前同步API对 PureSubs 的典型场景（15分钟-3小时）已足够，但异步API会显著提升长音频处理的用户体验。
