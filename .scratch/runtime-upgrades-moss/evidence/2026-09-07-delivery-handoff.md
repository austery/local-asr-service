...
Status: Reviewable local candidates. Quality acceptance and Lei's MOSS decision remain open; this is not production promotion.
...
| Upgrade MLX Audio with unchanged Qwen checkpoint and shared-runtime verification | [MLX report](2026-09-07-runtime-validation.md); 308 tests, static checks, eight identical texts, real contracts, rollback and lifecycle | Tested behavior preserved with three unreviewed speaker intervals; full quality acceptance is still open |
| Evaluate MOSS on English long-form multi-speaker audio | [MOSS report](2026-09-07-moss-probe.md); short, medium and one-hour upstream runs using one pinned BF16 checkpoint | Current pairing fails completeness at 36:19 of 60:00; No-Go recommended, Lei's decision pending |
...
| Independently upgrade FunASR with ASR/VAD/punctuation/CAM++ checkpoints fixed | [FunASR report](2026-09-07-funasr-preflight.md); clean install, 308 tests, eight real requests, unchanged hashes, preserved Qwen/pipeline contracts | Executable candidate; changed recognition text and repeated cost increases leave acceptance open |
...
1. Record Lei's current MOSS verdict: No-Go for the tested pairing, or prioritize investigating its early termination.
2. Review the three MLX/Paraformer speaker intervals and the changed FunASR transcripts using real audio and human references. Do not turn model agreement into ground truth or waive the agreed quality criteria implicitly.
3. Decide whether the repeated FunASR cost increases are justified by reviewed quality, or retain 1.2.7.
4. Approve the final runtime selection and empty retirement proposal. Show the selected diff and obtain explicit approval before any new push; main merge or deployment remains a separate action.
